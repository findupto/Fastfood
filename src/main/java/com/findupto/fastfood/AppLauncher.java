...
/** Application entry point: initializes the operational database, then authenticates the user. */
public final class AppLauncher {
    ...
    private static void login(){
        ...
        JPanel x=new JPanel(new java.awt.GridLayout(0,2,6,6));
        x.add(new JLabel("Username"));x.add(u);
        x.add(new JLabel("Password"));x.add(p);
        ...
        try(Connection c=DriverManager.getConnection(DB);
            PreparedStatement q=c.prepareStatement("SELECT role FROM users WHERE username=? AND password=? AND active=1")){
            q.setString(1,user);q.setString(2,hash(pass));
            try(ResultSet r=q.executeQuery()){
                if(r.next()){
                    CompletePOS app=new CompletePOS(user,r.getString(1));
                    app.setVisible(true);
                    return;
                }
            }
